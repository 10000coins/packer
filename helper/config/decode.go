...
func Decode(target interface{}, config *DecodeOpts, raws ...interface{}) error {
	...
	log.Printf("Megan raws is %#v", raws)
	...
	for _, raw := range raws {
		...
		log.Printf("Megan b")
		log.Printf("Megan raw is %#v", raw)
		// If we have unused keys, it is an error
		if len(md.Unused) > 0 {
			var err error
			sort.Strings(md.Unused)
			for _, unused := range md.Unused {
				log.Printf("Megan unused is %#v", unused)
				...
				// try to cast raw to a map[string]interface so we can retrieve
				// the type. If we can't, then we can't reason about whether the
				// key is actually fixable, so we will just return.
				rawCast, ok := raw.(map[string]interface{})
				if !ok {
				...
				if pluginType != nil {
					for k, deprecatedOptions := range DeprecatedOptions {
						...
						// all builders
						if glob.Glob(k, pluginType.(string)) {
